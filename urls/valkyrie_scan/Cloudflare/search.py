...
async def obter_capitulos(driver, url, inicio, fim, debug_var, baixando_label, app_instance, max_attent):
    ...
    try:
        btn = await pagina.find('btn btn-primary btn-adult-confirm')
        await btn.click()
    except:
        pass
    ...
    while True:
        ...
        while attempts < max_attent:
            try:
                await pagina.find('main version-chap no-volumn active', timeout = 2)
                break
            except:
                attempts += 1
                await pagina.reload()
        ...
        capitulos = await pagina.find('main version-chap no-volumn active')
