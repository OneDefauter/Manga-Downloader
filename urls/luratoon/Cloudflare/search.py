...
async def obter_capitulos(driver, url, inicio, fim, debug_var, baixando_label, app_instance, max_attent):
    ...
    while True:
        ...
        while attempts < max_attent:
            try:
                await pagina.find('capitulos__obra', timeout = 2)
                break
            except:
                attempts += 1
                await pagina.reload()
        
        capitulos = await pagina.find('capitulos__obra')
    
        for capitulo in capitulos.children[1].children:
            link = capitulo.children[0].attributes[1] if 'https://luratoon.com/' in capitulo.children[0].attributes[1] else capitulo.attributes[1]
