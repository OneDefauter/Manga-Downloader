...
import urls.slimeread.Cloudflare.search as obter_capitulos
import urls.slimeread.Cloudflare.login as login
import urls.slimeread.Cloudflare.run as run
...
async def setup(
    ...
    su_ = await login.login(driver, username, password)
    if not su_:
        return 'e005'
